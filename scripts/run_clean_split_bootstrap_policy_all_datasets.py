...
def run_one_dataset(
    ...
    # 7. Train bootstrap ensemble on train only
    ensemble = fit_bootstrap_ensemble(
        train_df=train_df,
        feature_cols=feature_cols,
        target_col="RUL",
        unit_col="unit",
        n_models=n_bootstrap_models,
    ...
    # 8. Predict mean/std
    cal_pred_mean, cal_pred_std = predict_bootstrap_mean_std(
        ensemble=ensemble,
        X=X_cal,
    )

    eval_pred_mean, eval_pred_std = predict_bootstrap_mean_std(
        ensemble=ensemble,
        X=X_eval,
    )
    ...
    # 10. Save uncertainty result frames
    ...
    # 11. Save uncertainty bins on evaluation split
    ...
    # 12. Choose k on calibration and evaluate on evaluation
    all_cal_sweeps = []
    clean_summary_rows = []
...
def main():
    ...
    parser.add_argument(
        ...
        default="gbr",
        choices=["gbr", "rf"],
        help="Bootstrap base model type: gbr or rf",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of bootstrap models",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Fraction of engines sampled per bootstrap model",
    )

    args = parser.parse_args()

    method_name = f"bootstrap_{args.model_type}"

    all_results = []

    for dataset_name in DATASETS:
        dataset_result = run_one_dataset(
            dataset_name=dataset_name,
            method_name=method_name,
            model_type=args.model_type,
            n_bootstrap_models=args.n_models,
            bootstrap_sample_frac=args.sample_frac,
        )
